...
RSpec.describe Zadarma::Resources::DirectNumbers do
  ...
  describe '#all' do
    it 'returns the list of direct numbers' do
      ...
      expect(subject.all).to eq('info' => [{'number' => '1234567890'}])
    end
  end

  describe '#countries' do
    it 'returns the list of countries' do
      stub_request(:get, /api.zadarma.com\/v1\/direct_numbers\/countries\//)
        .to_return(body: { status: 'success', info: [] }.to_json)

      response = client.direct_number_countries
  ...
  describe '#country' do
    it 'returns the list of destinations in a country' do
      stub_request(:get, /api.zadarma.com\/v1\/direct_numbers\/country\//)
        .to_return(body: { status: 'success', info: [] }.to_json)

      response = client.direct_number_country(country: 'US')
      expect(response['status']).to eq('success')
    end
  end
  ...
  describe '#available' do
    it 'returns the list of available numbers' do
      stub_request(:get, /api.zadarma.com\/v1\/direct_numbers\/available\/123\//)
        .to_return(body: { status: 'success', numbers: [] }.to_json)

      response = client.available_direct_numbers(direction_id: '123')
      expect(response['status']).to eq('success')
    end
  end
  ...
  describe '#order' do
    it 'orders a new direct number' do
      stub_request(:post, /api.zadarma.com\/v1\/direct_numbers\/order\//)
        .to_return(body: { status: 'success', number: '1234567890' }.to_json)

      response = client.order_direct_number(direction_id: '123')
      expect(response['status']).to eq('success')
  ...
  describe '#prolong' do
    it 'prolongs a direct number' do
      stub_request(:post, /api.zadarma.com\/v1\/direct_numbers\/prolong\//)
        .to_return(body: { status: 'success' }.to_json)

      response = client.prolong_direct_number(number: '1234567890', months: 1)
      expect(response['status']).to eq('success')
    end
  end
end
