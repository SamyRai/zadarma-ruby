...
RSpec.describe Zadarma::Resources::Info do
  subject(:info) { described_class.new(client: client) }
  ...
  describe '#balance' do
    it 'returns the current balance' do
      ...
      expect(info.balance).to eq('balance' => 100.0)
    end
  end
  ...
  describe '#price' do
    it 'returns the price for a given number' do
      ...
      expect(info.price(number: '1234567890')).to eq('price' => 0.01)
    end
    ...
    it 'returns the price for a given number and caller_id' do
      ...
      expect(info.price(number: '1234567890', caller_id: '0987654321')).to eq('price' => 0.02)
    end
  end
end
